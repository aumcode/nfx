  ...
  public class CellElement : Element
  {
     ...
     #region Protected
       ...
       protected override void RegionChanged()
       {
         ...
         {
           m_ColumnResizeElement.Region = new Rectangle(Region.Right - RESIZE_WIDTH, Region.Top+1, RESIZE_WIDTH, Region.Height-2);
         }
       }
       ...
       protected virtual void PaintValue(Graphics gr, object value)
       {
           using (StringFormat fmt = new StringFormat())
           {
             ...
             if (ha==HAlignment.Left|| ha==HAlignment.Right)  fmt.FormatFlags &= ~StringFormatFlags.DirectionRightToLeft;
             
             switch(ha)
             {
               case HAlignment.Left: fmt.Alignment = StringAlignment.Near; break;
               case HAlignment.Center: fmt.Alignment = StringAlignment.Center; break;
               case HAlignment.Right: fmt.Alignment = StringAlignment.Far; break;
               case HAlignment.Near: fmt.Alignment = StringAlignment.Near; break;
               case HAlignment.Far: fmt.Alignment = StringAlignment.Far; break;
               default: fmt.Alignment = StringAlignment.Near; break;
             }
             ...
             gr.DrawString(str, m_Style.Font, Brushes.Black, reg, fmt);
           } 
       }
     ...
     #region .pvt .impl
          ...
          private Style getBaseStyle()
          {
            var baseStyle = m_Column.HeaderStyle;
             ...
             {
               var grid = m_Grid;
               var columnSelectionStyled = grid.MultiSelect || !grid.CellSelectionAllowed || m_Column.HasCellSelection;
          ...
          private Brush getBrush(Style style)
          {
            switch(style.BGKind)
            {
              case BGKind.Hatched:
                      {
                        var result = new HatchBrush(style.BGHatchStyle, style.BGHatchColor, style.BGColor);
                        return result;
                      }
                      
              case BGKind.VerticalGradient:
                      {
                        var result = new LinearGradientBrush(Region, style.BGColor, style.BGColor2,LinearGradientMode.Vertical);
                        return result;
                      }
              case BGKind.HorizontalGradient:
                      {
                        var result = new LinearGradientBrush(Region, style.BGColor, style.BGColor2,LinearGradientMode.Horizontal);
                        return result;
                      }               
              case BGKind.ForwardDiagonalGradient:
                      {
                        var result = new LinearGradientBrush(Region, style.BGColor, style.BGColor2,LinearGradientMode.ForwardDiagonal);
                        return result;
                      }
              case BGKind.BackwardDiagonalGradient:
                      {
                        var result = new LinearGradientBrush(Region, style.BGColor, style.BGColor2,LinearGradientMode.BackwardDiagonal);
                        return result;
                      }                  
