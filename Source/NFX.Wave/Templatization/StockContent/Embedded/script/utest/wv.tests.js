       ...
       run("Strings", "siNum 0 <= n < 1", function () {
         ...
         assertTrue("100.00m", WAVE.siNum(.1));
         assertTrue("-10.010m", WAVE.siNum(-.01001, 3));
         assertTrue("1.2m", WAVE.siNum(.00118, 1));
         assertTrue("10µ", WAVE.siNum(.00001, 0));
       });
       ...
       run("Key", "genAutoincKey-CTR1", function(){
          for(var i=0; i<5; i++)
          {
           var counter = WAVE.genAutoincKey("CTR1",5)
           logi( counter ); 
          }
       ...
       run("Key", "genAutoincKey-dfltArgs", function(){
          for(var i=0; i<100; i++)
          {
           var counter = WAVE.genAutoincKey()
           logi( counter ); 
          }
       ...
       run("Geometry", "intersectionLengthRectLineXY", function(){
          ...
          assertTrue(5 == WAVE.Geometry.intersectionLengthRectLineXY(0, 0, 4, 3, -10, 4, 6, -.78));
    ...
    run("EventManager", "event-with-var-args-1", function(){
     ...
     run("Walker", "wSawGen-symmetry-.5", function() {
          var saw = WAVE.signalConstSrc().wSawGen({s: .5, r: 4});
     ...
     run("Walker", "wSquareGen-symmetry-.5", function() {
          var square = WAVE.signalConstSrc().wSquareGen({s: .5, r: 40});
     ...
     run("Walker", "wConstLinearSampling-step-.5", function() {
          var w = WAVE.arrayWalkable([{s: 0, a: 10}, {s: 1, a: 20}, {s: 2, a: 10}]).wConstLinearSampling(.5);
