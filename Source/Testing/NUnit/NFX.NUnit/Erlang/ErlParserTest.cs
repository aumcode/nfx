    ...
    public class ErlParserFixture
    {
        ...
        public void ErlParserMFATest()
        {
            var tests = new Dictionary<string, Tuple<string, string, ErlList, object[]>> {
                ...
                { "a:b(~w).",   Tuple.Create("a", "b", new ErlList(10), new object[]{10} )},
                { "a:b(~f,~d).",Tuple.Create("a", "b", new ErlList(10d,20), new object[]{10d,20} )},
                { "a:b([~w,~w],30)", Tuple.Create("a", "b", new ErlList(new ErlList(10,20),30), new object[]{10,20} )},
                { "a:b([~w,~w],~w)", Tuple.Create("a", "b", new ErlList(new ErlList(10,20),30), new object[]{10,20,30} )},
            };
            ...
            foreach (var t in tests)
            {
                ...
                Assert.AreEqual(t.Value.Item1, res.Item1.Value,
                    "Unexpected module value: {0} (expected: {1})".Args(res.Item1.Value, t.Value.Item1));
                Assert.AreEqual(t.Value.Item2, res.Item2.Value,
                    "Unexpected function value: {0} (expected: {1})".Args(res.Item2.Value, t.Value.Item2));
                Assert.AreEqual(t.Value.Item3, res.Item3,
                    "Unexpected args value: {0} (expected: {1})".Args(res.Item3, t.Value.Item3));
            }
        }
        ...
        public void ErlParserMFAFailTest()
        {
            ...
            foreach (var t in tests)
                Assert.Throws<ErlException>(
                    () => ErlObject.ParseMFA(t.Key, t.Value), "Errorneously parsing term: {0}".Args(t.Key));
        }
        ...
        public void ErlFormatTest()
        {
            var tests = new Dictionary<string, ErlList> {
                ...
                { "{ok, A::a()}",   new ErlList("{ok, ~v::a()}", new ErlVar("A", ErlTypeOrder.ErlLong)) },
