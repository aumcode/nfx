...
namespace NFX.Instrumentation.Analytics
{
    ...
    /// </summary>
    public class TimeHistogram<TData2> : ITimeHistogram
    {
        private class timeFrame : IDisposable
        {
            public timeFrame(TimeHistogram<TData2> hist, TData2 dim2Value)
            {
                m_Hist = hist;
                m_Dim2Value = dim2Value;
                m_Hist.Start();
            }

            public void Dispose()
            {
                m_Hist.Stop(m_Dim2Value);
            }

            TimeHistogram<TData2>   m_Hist;
            TData2                  m_Dim2Value;
        }
        ...
        #region Public/Props
            ...
            /// </summary>
            public IDisposable TimeFrame(TData2 dim1Value)
            {
                return new timeFrame(this as TimeHistogram<TData2>, dim1Value);
            }
            ...
            /// </summary>
            public void Stop(TData2 dim2Value)
            {
                double elapsed = m_Timer.Stop();
                m_Hist.DoSample(m_Hist.Keys(elapsed, dim2Value));
            }

            /// <summary>
            /// Records time measurement in seconds
            /// </summary>
            public void Sample(double secInterval, TData2 dim2Value)
            {
                m_Hist.Sample(secInterval, dim2Value);
            }
        ...
        #region Private
            ...
            private Histogram<double, TData2> m_Hist;
